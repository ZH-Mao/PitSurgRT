...
# Has vitually confirmed, should be correct 20231008
...
from .mmhausdorff_utilis import get_class_weight, weighted_loss
...
def compute_dtm(img_gt: Tensor, pred: Tensor) -> Tensor:
    ...
    field = torch.zeros_like(pred)
    out_shape = pred.shape
    for b in range(out_shape[0]):  # batch size
        for c in range(1, out_shape[1]):  # default 0 channel is background
            # posmask = img_gt[b].byte()
            posmask = (img_gt[b].byte() == c).long()  # zhehua
            negmask = 1 - posmask  # zhehua
            if posmask.any():
                posdis = distance(posmask)
                negdis = distance(negmask)  # zhehua
                field[b][c] = torch.from_numpy(
                    posdis) + torch.from_numpy(negdis)  # zhehua
...
def hd_loss(seg_soft: Tensor,
    ...
    seg = torch.argmax(seg_soft.cpu(), dim=1, keepdim=False).long()
    seg = seg.to(seg_soft.device)
    if class_weight is not None:
        assert class_weight.ndim == num_class
    for i in range(1, num_class):
        if i != ignore_index:
            # delta_s = (seg_soft[:, i, ...] - gt.float())**2
            seg_class_i = (seg.byte() == i).long()  # zhehua
            gt_i = (gt.byte() == i).long()  # zhehua
            delta_s = (seg_class_i.float() - gt_i.float())**2 #zhehua
            s_dtm = seg_dtm[:, i, ...]**2
            g_dtm = gt_dtm[:, i, ...]**2
            dtm = s_dtm + g_dtm
...
if __name__ == '__main__':
    from mmhausdorff_utilis import plot, pad_with, summary
    import numpy as np
    # preparing dummy test data
    x = np.ones((6, 6))
    x = np.pad(x, 10, pad_with)
    y = np.roll(np.roll(x, 1, axis=0), 1, axis=1)

    # plot(x, y)

    HD_dt = HuasdorffDisstanceLoss(ignore_index=1)
