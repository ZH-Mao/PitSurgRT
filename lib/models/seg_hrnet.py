...
class HighResolutionNet(nn.Module):
    ...
    def forward(self, x):  # [1, 3, 736, 1280]
        x = self.conv1(x)
        x = self.bn1(x)  # [1, 64, 368, 640]
        x = self.relu(x)
        x = self.conv2(x)  # [1, 64, 184, 320]
        x = self.bn2(x)
        x = self.relu(x)
        x = self.layer1(x)  # [1, 256, 184, 320]
        ...
        for i in range(self.stage2_cfg['NUM_BRANCHES']):
            if self.transition1[i] is not None:
                ...
                x_list.append(x)
        # y_list[0].shape=[1, 48, 184, 320] y_list[1].shape=[1, 96, 92, 160]
        ...
        for i in range(self.stage4_cfg['NUM_BRANCHES']):
            if self.transition3[i] is not None:
                ...
                x_list.append(y_list[i])
        # x[0].shape = [1, 48, 184, 320]
        # x[1].shape = [1, 96, 92, 160]
        # x[2].shape = [1, 192, 46, 80]
        # x[3].shape = [1, 384, 23, 40]
        ...
        # x.shape = [1, 720, 184, 320]
        x = torch.cat([x[0], x1, x2, x3], 1)

        # x.shape = [1, 3, 184, 320]
        x_seg = self.last_layer(x)
